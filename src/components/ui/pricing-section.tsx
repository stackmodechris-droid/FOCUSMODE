...
export function PricingSection() {
  const discount = SITE.discountPercent ?? 40;
    ...
    <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-14">
      <div className="relative flex items-center justify-center">
        <div className="premium-product-stage max-w-[340px]">
          <ProductGlow src="/products/bottle-front.png" alt="Focus Mode supplement bottle" size={460} glow="dual" priority className="max-w-[260px] sm:max-w-[320px] md:max-w-none" />
      ...
      <motion.div
        ...
        className="premium-card relative overflow-hidden border-bolt/30 p-8 md:p-10 discount-pulse"
      >
        <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-bolt/5 blur-3xl" />
        ...
        <h2 className="mt-4 font-heading text-3xl font-extrabold leading-tight text-white md:text-4xl">
          Focus Mode <span className="text-bolt text-glow-bolt">Cognitive Formula</span>
        </h2>

        {/* High-converting price block with anchoring + scarcity */}
        <div className="mt-6 flex flex-wrap items-baseline gap-x-4 gap-y-1">
          <div>
            <span className="font-heading text-6xl font-extrabold text-white">${SITE.price}</span>
        ...
        <p className="mt-1 text-sm text-bolt font-medium">Limited Time Operator Discount — Ends When Timer Hits Zero</p>
        ...
        <ul className="mt-6 space-y-2.5">
          {INCLUDES.map((item) => (
            <li key={item} className="flex items-center gap-3 text-sm text-silver/85">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center bg-bolt/15 text-bolt">
        ...
        <div className="mt-5 flex items-center gap-2 rounded border border-bolt/30 bg-bolt/10 px-4 py-3 text-xs text-bolt">
        ...
        <div className="mt-7 border-t border-white/10 pt-7">
          ...
          <a
            ...
            className="mt-3 block text-center font-mono-data text-[10px] uppercase tracking-widest text-silver/50 hover:text-bolt underline-offset-4 hover:underline"
          >
